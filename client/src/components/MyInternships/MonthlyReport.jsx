...
function generatePDF(report, studentName, studentId) {
  ...
  if (report.skillsLearned?.length > 0) {
    let chipX = 14, chipY = afterTable + 12;
    const chipH = 7;
    doc.setFontSize(9);
...
export default function MonthlyReport({ internshipId, supervisorEmail, supervisorName }) {
    ...
    getReports(internshipId)
      ...
      .catch(() => {
        Swal.fire({
          ...
          text: "Could not load monthly reports. Please refresh.",
  ...
  const openEdit = (report) => {
    ...
    setForm({
      monthName,
      monthYear,
      weekSummary:  report.weekSummary ?? { week01: "", week02: "", week03: "", week04: "" },
  ...
  const handleSubmit = async (e) => {
    ...
    const isDuplicate   = reports.some((r) => r.month === combinedMonth && r._id !== editId);
    ...
    try {
      if (editId) {
        ...
        setReports((prev) => prev.map((r) => (r._id === editId ? res.data : r)));
        await Swal.fire({
          icon: "success",
          title: "Report Updated!",
          text: `Your report for ${combinedMonth} has been updated.`,
          confirmButtonColor: "#3B6FE8",
          timer: 2000,
        ...
        await Swal.fire({
          icon: "success",
          title: "Report Saved!",
          text: `Your report for ${combinedMonth} has been added.`,
          confirmButtonColor: "#3B6FE8",
          timer: 2000,
          timerProgressBar: true,
        });
  ...
  const handleDelete = async (id, month) => {
    const result = await Swal.fire({
      icon: "warning",
      title: "Delete Report?",
      text: `The report for "${month}" will be permanently deleted.`,
    ...
    try {
      ...
      Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `The report for "${month}" has been removed.`,
        confirmButtonColor: "#3B6FE8",
        timer: 2000,
        timerProgressBar: true,
      });
  ...
  const handleDownloadPDF = (report) => {
    ...
    try {
      ...
      setTimeout(() =>
        setPdfStatus((prev) => ({ ...prev, [report._id]: null })), 2500
      );
    } catch (err) {
  ...
  const handleSendEmail = async (report) => {
    ...
    // Confirm before downloading + opening email
    const result = await Swal.fire({
      icon: "info",
      title: "Send Report?",
      html: `This will <strong>download the PDF</strong> and open your email app with <strong>${email}</strong> pre-filled.<br/><br/>Please attach the downloaded PDF before sending.`,
      showCancelButton: true,
    ...
    try {
      ...
      setTimeout(() =>
        setEmailSent((prev) => ({ ...prev, [report._id]: false })), 4000
      );
    } catch (err) {
      Swal.fire({
        icon: "error",
  ...
  const handleCancelForm = async () => {
    const hasData =
      form.monthName !== "" ||
      form.skillsLearned !== "" ||
      Object.values(form.weekSummary).some((v) => v !== "");

    if (hasData) {
      const result = await Swal.fire({
        icon: "question",
        title: "Discard Changes?",
        text: "Your unsaved report data will be lost.",
        showCancelButton: true,
  ...
  const clearFilters = () => { setSearch(""); setSortBy("newest"); };
  ...
  const filteredReports = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(
        (r) =>
          ...
          Object.values(r.weekSummary || {}).some((w) => w?.toLowerCase().includes(q)) ||
    ...
    if (sortBy === "newest") {
      ...
      result.sort((a, b) => (b.skillsLearned?.length ?? 0) - (a.skillsLearned?.length ?? 0));
    }
    return result;
  ...
  const selectCls    = "w-full rounded-xl border border-[#E8EAF0] bg-white px-4 py-2.5 text-sm text-[#1A1D27] outline-none focus:border-[#3B6FE8] transition-colors cursor-pointer";
  const selectErrCls = "w-full rounded-xl border border-[#DC2626] bg-white px-4 py-2.5 text-sm text-[#1A1D27] outline-none transition-colors cursor-pointer";
    ...
    <div>
        ...
        <div className="mb-4 flex flex-wrap gap-3">
          ...
          <div className="rounded-xl border border-[#E8EAF0] bg-white px-4 py-2.5">
            ...
            <p className="text-lg font-bold text-[#7C3AED]">
              {reports.reduce(
                (sum, r) => sum + Object.values(r.weekSummary || {}).filter(Boolean).length, 0
              )}
            </p>
          </div>
        </div>
        ...
        <div className="mb-5 space-y-3">
          ...
          {search && (
            <p className="text-xs text-[#6B7280]">
              Showing{" "}
              <span className="font-semibold text-[#1A1D27]">{filteredReports.length}</span>
              {" "}of{" "}
              <span className="font-semibold text-[#1A1D27]">{reports.length}</span>
              {" "}reports
            </p>
          )}
        </div>
      ...
      {showForm && (
        <div className="mb-6 rounded-2xl border border-[#E8EAF0] bg-white p-6 shadow-[0_1px_4px_rgba(0,0,0,0.04)]">
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              ...
              {monthError && (
                <div className="mt-2 flex items-center gap-2 rounded-xl border border-[#DC2626] bg-[#FEE2E2] px-4 py-2.5">
                  ...
                  <p className="text-xs font-semibold text-[#DC2626]">{monthError}</p>
                </div>
              )}
              {form.monthName && (
            ...
            <p className="mb-2 text-xs font-semibold text-[#1A1D27]">Week Summaries</p>
      ...
      {reports.length > 0 && filteredReports.length === 0 && (
        <div className="rounded-2xl border border-dashed border-[#D4E0FA] bg-[#F7F8FA] p-8 text-center">
          <p className="text-sm font-semibold text-[#1A1D27]">No reports found</p>
          <p className="mt-1 text-xs text-[#6B7280]">Try a different search term</p>
          <button onClick={clearFilters}
              ...
              {["week01", "week02", "week03", "week04"].map((wk, i) =>
                r.weekSummary?.[wk] ? (
                  <div key={wk} className="rounded-xl bg-[#F7F8FA] p-3">
                    <p className="mb-1 text-xs font-semibold text-[#3B6FE8]">Week {i + 1}</p>
                    <p className="text-xs leading-relaxed text-[#6B7280]">{r.weekSummary[wk]}</p>
                  </div>
                ) : null
              )}
            </div>
            ...
            {emailBox[r._id] && (
              <div className="mt-3 rounded-2xl border border-[#E8EAF0] bg-[#F7F8FA] p-4">
                ...
                <p className="mb-3 text-xs text-[#6B7280]">
                  This will download the PDF and open your email app with the address pre-filled.
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setEmailInputs((prev) => ({ ...prev, [r._id]: e.target.value }))
                    }
                  ...
                  <button
                    onClick={() => setEmailBox((prev) => ({ ...prev, [r._id]: false }))}
